...
var dollhouse, raycaster, download_data, scan;
...
function next() {
  ...
  matt.load_mesh(scan, mesh, function(object){
    dollhouse = object;
//    dollhouse.position.x = SIZE_X / 2;
//    dollhouse.position.y = SIZE_Y / 2;
    scene.add( object );
    render();
    load_connections(scan);
//    fitCameraToCenteredObject(camera, dollhouse);
  })
...
function init() {
  ...
  renderer = new THREE.WebGLRenderer({alpha: true, canvas: document.getElementById("skybox"),
...
function fitCameraToCenteredObject(camera, house) {

  const boundingBox = new THREE.Box3();

  boundingBox.setFromObject(house);
...
function add_connection(obj1, obj2) {
  ...
  if (!line){
    var material = new THREE.LineBasicMaterial({ color: CONNECTION, linewidth: 20 });
