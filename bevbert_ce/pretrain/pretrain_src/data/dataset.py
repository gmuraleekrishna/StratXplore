...
class ReverieTextPathData(object):
    def __init__(
        self, anno_files, img_ft_file, dep_ft_file, obj_ft_file, img_grid_file, dep_grid_file,
        ...
        self.img_grid_file = img_grid_file
        self.dep_grid_file = dep_grid_file
    ...
    def get_scanvp_grid_feature(self, scan, viewpoint):
        key = '%s_%s' % (scan, viewpoint)
        with h5py.File(self.img_grid_file, 'r') as f:
            rgbs = f[key][...].astype(np.float32)
        with h5py.File(self.dep_grid_file, 'r') as f:
    ...
    def get_bev_inputs(self, scan, cur_vp, cur_heading, cur_elevation, cand_vpids):
        ...
        return rgbs, depths, T_c2w, T_w2c, S_w2c, bev_cand_idxs


class R2RTextPathData(ReverieTextPathData):
    def __init__(
        self, anno_files, img_ft_file, dep_ft_file, 
        img_grid_file, dep_grid_file,
        scanvp_cands_file, connectivity_dir,
        image_feat_size=2048, image_prob_size=1000, depth_feat_size=128, angle_feat_size=4,
        max_txt_len=100, in_memory=True, act_visited_node=False,
        val_sample_num=None
        ...
        super().__init__(
            anno_files, img_ft_file, dep_ft_file, None, img_grid_file, dep_grid_file,
            scanvp_cands_file, connectivity_dir,
    ...
    def get_input(
        ...
        # local: the first token is [stop]
        rgbs, depths, T_c2w, T_w2c, S_w2c, bev_cand_idxs = \
            self.get_bev_inputs(scan, end_vp, cur_heading, cur_elevation, traj_cand_vpids[-1])
        bev_gpos_fts = self.get_gmap_pos_fts(scan, end_vp, [start_vp], cur_heading, cur_elevation)
