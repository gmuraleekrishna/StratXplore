...
def set_task_ratio(data_cfg, opts):
    assert opts.task_ratio is not None
    task_ratio = opts.task_ratio.split('.')
    tasks = task_ratio[::2]
    ratios = task_ratio[1::2]
    ratios = [int(x) for x in ratios]

    data_cfg.tasks = tasks
    data_cfg.mix_ratio = ratios
    LOGGER.info(f'Prefix tasks: {tasks}, Task ratios: {ratios}')
    return data_cfg
