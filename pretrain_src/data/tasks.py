...
class MlmDataset(Dataset):
    def __init__(self, nav_db, tok, end_vp_pos_ratio):
        self.nav_db = nav_db
        self.tok = tok
    ...
    def __getitem__(self, idx):
        ...
        inputs = self.nav_db.get_input(idx, end_vp_type)
        ...
        output['rgbs'] = torch.from_numpy(inputs['rgbs']).reshape(12, 14, 14, 768)
        output['depths'] = torch.from_numpy(inputs['depths']).reshape(12, 1, 14, 14)
        output['sems'] = torch.from_numpy(inputs['sems'])
        output['T_c2w'] = torch.from_numpy(inputs['T_c2w'])
        output['T_w2c'] = torch.from_numpy(inputs['T_w2c'])
        output['S_w2c'] = torch.from_numpy(inputs['S_w2c'])
        output['bev_masks'] = torch.ones(BEV_DIM * BEV_DIM).bool()
        output['bev_nav_masks'] = torch.zeros(BEV_DIM * BEV_DIM).bool()
        output['bev_nav_masks'][inputs['bev_cand_idxs']] = True
        output['bev_cand_idxs'] = torch.from_numpy(inputs['bev_cand_idxs'])
        output['bev_gpos_fts'] = torch.from_numpy(inputs['bev_gpos_fts'])
        return output
...
def mlm_collate(inputs):
    ...
    # bev batches
    batch['rgbs'] = pad_tensors(batch['rgbs'])
    batch['depths'] = pad_tensors(batch['depths'])
    batch['sems'] = pad_tensors(batch['sems'])
    batch['T_c2w'] = pad_tensors(batch['T_c2w'])
    batch['T_w2c'] = pad_tensors(batch['T_w2c'])
    batch['S_w2c'] = pad_tensors(batch['S_w2c'])
    batch['bev_masks'] = pad_tensors(batch['bev_masks'])
    batch['bev_nav_masks'] = pad_tensors(batch['bev_nav_masks'])
    batch['bev_cand_idxs'] = pad_tensors(batch['bev_cand_idxs'])
    batch['bev_gpos_fts'] = pad_tensors(batch['bev_gpos_fts'])
...
class SapDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        output = {'txt_ids': torch.LongTensor(inputs['instr_encoding']),
                  'traj_view_img_fts': [torch.from_numpy(x) for x in inputs['traj_view_img_fts']]}
...
class OGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        output = {'txt_ids': torch.LongTensor(inputs['instr_encoding']),
                  'traj_view_img_fts': [torch.from_numpy(x) for x in inputs['traj_view_img_fts']],
                  'traj_obj_img_fts': [torch.from_numpy(x) for x in inputs['traj_obj_img_fts']],
                  'traj_loc_fts': [torch.from_numpy(x) for x in inputs['traj_loc_fts']],
                  'traj_nav_types': [torch.LongTensor(x) for x in inputs['traj_nav_types']],
                  'traj_cand_vpids': inputs['traj_cand_vpids'], 'traj_vpids': inputs['traj_vpids'],
                  'gmap_vpids': inputs['gmap_vpids'], 'gmap_step_ids': torch.LongTensor(inputs['gmap_step_ids']),
                  'gmap_visited_masks': torch.BoolTensor(inputs['gmap_visited_masks']),
                  'gmap_pos_fts': torch.from_numpy(inputs['gmap_pos_fts']),
                  'gmap_pair_dists': torch.from_numpy(inputs['gmap_pair_dists']),
                  'rgbs': torch.from_numpy(inputs['rgbs']).reshape(12, 14, 14, 768),
                  'depths': torch.from_numpy(inputs['depths']).reshape(12, 1, 14, 14),
                  'sems': torch.from_numpy(inputs['sems']), 'T_c2w': torch.from_numpy(inputs['T_c2w']),
                  'T_w2c': torch.from_numpy(inputs['T_w2c']), 'S_w2c': torch.from_numpy(inputs['S_w2c']),
                  'bev_masks': torch.ones(BEV_DIM * BEV_DIM).bool(),
                  'bev_nav_masks': torch.zeros(BEV_DIM * BEV_DIM).bool()}
...
############### Multi-label Semantic Prediction ###############
...
def sem_collate(inputs):
    ...
    batch['traj_view_img_fts'] = pad_tensors(sum(batch['traj_view_img_fts'], []))  # flatten list
    if 'traj_obj_img_fts' in batch:
        batch['traj_vp_obj_lens'] = torch.LongTensor(
            sum([[len(y) for y in x] for x in batch['traj_obj_img_fts']], [])
        )
...
############### Multi-label Mask Semantic Prediction ###############
class MaskSemDataset(Dataset):
    def __init__(self, nav_db, tok, mask_prob, end_vp_pos_ratio):
        '''Instruction Trajectory Matching'''
        self.nav_db = nav_db
        self.tok = tok
        self.mask_prob = mask_prob
    ...
    def __getitem__(self, idx):
        ...
        output['bev_mrc_masks'] = _get_img_mask(self.mask_prob, BEV_DIM * BEV_DIM)

        return output


def masksem_collate(inputs):
    batch = {
        k: [x[k] for x in inputs] for k in inputs[0].keys()
    }
    # text batches
    ...
    batch['bev_mrc_masks'] = pad_sequence(batch['bev_mrc_masks'], batch_first=True, padding_value=0)

    return batch


############### Perturbation/Correction Prediction ###############
class PerturbationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        inputs = self.nav_db.get_input(idx, 'pos', return_act_label=True, kidnap=True)
        ...
        output['path_types'] = torch.LongTensor(inputs['path_types'])
        return output


def perturbation_collate(inputs):
    batch = {
        k: [x[k] for x in inputs] for k in inputs[0].keys()
    }
    # text batches
    ...
    batch['path_types'] = pad_tensors(batch['path_types'], pad=-1)
    return batch
