...
def load_nav_graphs(connectivity_dir):
    ''' Load connectivity nav_graph for each scan '''

    def distance(pose1, pose2):
        ''' Euclidean distance between two nav_graph poses '''
...
def get_wrapped_window_indices(i, rows=3, cols=12):
...
wrapped_window_indices = [get_wrapped_window_indices(i) for i in range(36)]


def weighted_mean_around_index(panorama_features, i, centre_weight=0.8):
    indices = wrapped_window_indices[i]
