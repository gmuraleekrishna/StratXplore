...
from data.tasks import (
    ...
    MaskSemDataset, masksem_collate, PerturbationDataset, perturbation_collate, OGDataset, og_collate)
...
from utils.losses import multilabel_cross_entropy
...
def main(opts):
    ...
    val_nav_db = R2RTextPathData(
        ...
        angle_feat_size=model_config.angle_feat_size,
        max_txt_len=opts.max_txt_len, in_memory=True,
        val_sample_num=opts.val_sample_num, args=opts
    )
    ...
    if opts.checkpoint:
        ...
        saved_groups = optim_ckpt['optimizer']['param_groups']
        try:
    ...
    for step, (name, batch) in enumerate(meta_loader, start=global_step):
        ...
        if 'pert' in name:
            type_loss = model(batch, task=task, compute_loss=True)
            loss = opts.recovery_weight * type_loss.mean()
            n_loss_units['pert_type'] += type_loss.size(0)
        else:
            loss = model(batch, task=task, compute_loss=True)
            n_loss_units[name] += loss.size(0)
            loss = loss.mean()  # loss is not normalized in model
        ...
        # backward pass
        if parsed_args.gradient_accumulation_steps > 1:  # average loss
...
def validate_perturbation(model, val_loader):
    ...
    st = time.time()
    for i, batch in enumerate(val_loader):
        type_logits, path_types, recovery_label = model(batch, task='pert', compute_loss=False)
    ...
    LOGGER.info(
        f"validation finished in {int(tot_time)} seconds, pacc: {pacc * 100:.2f}, poacc: {poacc * 100:.2f}, pdacc: {pdacc * 100:.2f}, pracc: {pracc * 100:.2f}")
    return val_log
