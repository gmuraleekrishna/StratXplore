...
class ImageEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.object_label = nn.Embedding(3, config.hidden_size)
...
class GlobalMapEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.gmap_unique_object_embeddings = nn.Embedding(config.max_action_steps, config.hidden_size)
        self.encoder = CrossmodalEncoder(config)
...
class GlocalTextPathCMT(BertPreTrainedModel):
    ...
    def forward(
        ...
        )  # batch of traj embeds, [(T1, V1, D), (T2, V2, D)...]
        ...
        if traj_obj_img_fts is not None:
            ...
            obj_embeds = pad_tensors_wgrad([
                x[-1, view_len: view_len + obj_len] for x, view_len, obj_len in
                zip(split_traj_embeds, vp_view_lens, vp_obj_lens)
            ])
            obj_masks = gen_seq_masks(torch.stack(vp_obj_lens, 0))
        else:
            obj_embeds = obj_masks = None

        # bev embeds
        bev_embeds, obj_embeds = self.local_encoder(
            txt_embeds, txt_masks,
            bev_fts, bev_pos_fts, bev_masks, bev_nav_masks,
            obj_embeds, obj_masks,
        )

        return gmap_embeds, bev_embeds, obj_embeds, obj_masks
    ...
    def forward_mlm(
        ...
        # bev embeds
        bev_embeds = self.local_encoder.bev_input_embedding(bev_fts, bev_pos_fts, bev_nav_masks)
        if obj_embeds is not None:
            bev_obj_embeds = torch.cat([bev_embeds, obj_embeds], dim=1)
            bev_obj_masks = torch.cat([bev_masks, obj_masks], dim=1)
        else:
            bev_obj_embeds = bev_embeds
            bev_obj_masks = bev_masks
        extended_bev_obj_masks = extend_neg_masks(bev_obj_masks)

        bev_txt_embeds = txt_embeds
        for layer_module in self.local_encoder.encoder.x_layers:
            bev_txt_embeds = layer_module.forward_lang2visn(
                bev_txt_embeds, extended_txt_masks,
                bev_obj_embeds, extended_bev_obj_masks,
            )

        txt_embeds = gmap_txt_embeds + bev_txt_embeds
        return txt_embeds
    ...
    def forward_sem(
            ...
            traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
            gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids,
            bev_fts, bev_pos_fts, bev_masks, bev_nav_masks, sem_pred_token=None
    ):
        if sem_pred_token == 'cattn':
            # text embedding
            txt_token_type_ids = torch.zeros_like(txt_ids)
            txt_embeds = self.embeddings(txt_ids, token_type_ids=txt_token_type_ids)
            txt_masks = gen_seq_masks(txt_lens)
        ...
        elif sem_pred_token == 'sattn':
            bev_embeds = self.local_encoder.bev_input_embedding(bev_fts, bev_pos_fts, bev_nav_masks)
            extended_bev_masks = extend_neg_masks(bev_masks)
            for layer_module in self.local_encoder.encoder.x_layers:
                bev_embeds = layer_module.forward_visn2visn(bev_embeds, extended_bev_masks)

        elif sem_pred_token == 'embed':
            bev_embeds = self.local_encoder.bev_input_embedding(bev_fts, bev_pos_fts, bev_nav_masks)
