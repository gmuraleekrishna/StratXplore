...
from utils.losses import MultiLabelCrossEntropyLoss
...
class MulClsPrediction(nn.Module):
    def __init__(self, hidden_size, input_size=None, cls_dim=40):
        super().__init__()
        if input_size is None:
            input_size = hidden_size
        self.net = nn.Sequential(nn.Linear(input_size, hidden_size),
...
class GlocalTextPathCMTPreTraining(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if 'pert' in config.pretrain_tasks:
            self.global_cpt_head = MulClsPrediction(self.config.hidden_size, cls_dim=3)
            # self.global_confidence_head = ClsPrediction(self.config.hidden_size)
            self.mlce_criterion = MultiLabelCrossEntropyLoss(reduction='none')
    ...
    def forward(self, batch, task, compute_loss=True):
        ...
        if task.startswith('mlm'):
            ...
            return self.forward_mlm(
                ...
                batch['traj_step_lens'], batch['traj_vp_view_lens'], batch['traj_vp_obj_lens'], batch['traj_vpids'],
                batch['traj_cand_vpids'],
                batch['gmap_lens'], batch['gmap_step_ids'], batch['gmap_pos_fts'], batch['gmap_pair_dists'],
                batch['gmap_vpids'],
            ...
            return self.forward_sap(
                ...
                batch['traj_cand_vpids'],
                batch['gmap_lens'], batch['gmap_step_ids'], batch['gmap_pos_fts'], batch['gmap_pair_dists'],
                batch['gmap_vpids'], batch['gmap_visited_masks'],
                batch['bev_fts'], batch['bev_pos_fts'], batch['bev_masks'], batch['bev_nav_masks'],
                batch['bev_cand_idxs'],
            ...
            batch = self.lift_splat(batch)
            batch = self.drop_feats(batch)
            return self.forward_path_correction(
                batch['txt_ids'], batch['txt_lens'],
                batch['traj_view_img_fts'], batch['traj_obj_img_fts'], batch['traj_loc_fts'], batch['traj_nav_types'],
                batch['traj_step_lens'], batch['traj_vp_view_lens'], batch['traj_vp_obj_lens'], batch['traj_vpids'],
                batch['traj_cand_vpids'],
                ...
                batch['bev_fts'], batch['bev_pos_fts'], batch['bev_masks'], batch['bev_nav_masks'],
                batch['bev_cand_idxs'],
                batch['global_act_labels'], batch['local_act_labels'], batch['path_types'],
                compute_loss
            )
        else:
    ...
    def forward_path_correction(
            self, txt_ids, txt_lens, traj_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types,
            traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
            gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, gmap_visited_masks,
            bev_fts, bev_pos_fts, bev_masks, bev_nav_masks, bev_cand_idxs,
            global_act_labels, local_act_labels, path_types, compute_loss
    ):
        batch_size = txt_ids.size(0)
        ...
        type_logits = self.global_cpt_head(gmap_embeds[:, 1:path_types.shape[1] + 1]).squeeze(2)
        type_logits.masked_fill_(gmap_visited_masks[:, 1:path_types.shape[1] + 1, None].logical_not(), -float('inf'))

        # confidence_logits = self.global_confidence_head(gmap_embeds).squeeze(2)[:, 1:]
        # global_logits.masked_fill_(gen_seq_masks(gmap_lens).logical_not(), -float('inf'))

        if compute_loss:
            type_loss = self.mlce_criterion(type_logits, path_types)
            return type_loss
        else:
            return type_logits, path_types
