...
from r2r.agent_utils import panorama_windows
from parser import parsed_args as args
...
class R2RNavBatch(object):
    ...
    def __init__(
        ...
        with open(os.path.join(args.know_root_path, "pkls/detect_feat_genome_by_view.pkl"), 'rb') as f:
            self.detect_feat_genome_by_view = pickle.load(f)
        with open(os.path.join(args.know_root_path, 'pkls/vg_class_vico_embed.pkl'), 'rb') as f:
            self.vg_class_vico_embed = pickle.load(f)

        self.vg_class_vico_embed[-1] = torch.zeros(200)
    ...
    def _load_nav_graphs(self):
        """
        load nav_graph from self.scan,
        Store the nav_graph {scan_id: nav_graph} in self.graphs
        Store the shortest path {scan_id: {view_id_x: {view_id_y: [path]} } } in self.paths
        Store the distances in self.distances. (Structure see above)
        Load connectivity nav_graph for each scan, useful for reasoning about shortest paths
        :return: None
        """
    ...
    def _get_obs(self):
        ...
        for i, (feature, rgb, depth, state) in enumerate(self.env.getStates()):
            ...
            detect_label = self.detect_feat_genome_by_view[f"{state.scanId}_{state.location.viewpointId}"]
            # Full features
            candidate = self.make_candidate(feature, state)

            # [visual_feature, angle_feature] for views
            feature = np.concatenate((feature, self.angle_feature[base_view_id]), -1)
            neighbour_glove_embeds = {}
            neighbour_detect_labels = {}
            all_neighbour_objects = set()
            for cc in candidate:
                cc_vp_key = f"{state.location.viewpointId}_{cc['viewpointId']}"
                if cc_vp_key not in self._knowledge_cache:
                    dir_objects = detect_label[cc['pointId']]
                    if len(dir_objects) > 0:
                        obj_ids = set(sum([obj[:3].astype(int).tolist() for obj in dir_objects], []))
                        neighbour_detect_labels[cc['viewpointId']] = obj_ids
                        all_neighbour_objects.update(obj_ids)
                        embeds = [self.vg_class_vico_embed[int(detect[0])] for detect in detect_label[cc['pointId']]]
                        neighbour_glove_embeds[cc['viewpointId']] = torch.vstack(embeds).sum(0)
                    else:
                        neighbour_detect_labels[cc['viewpointId']] = set()
                        neighbour_glove_embeds[cc['viewpointId']] = torch.zeros(200)
                    self._knowledge_cache[cc_vp_key] = (neighbour_glove_embeds, neighbour_detect_labels)
                else:
                    neighbour_glove_embeds, neighbour_detect_labels = self._knowledge_cache[cc_vp_key]
            if state.location.viewpointId not in self._knowledge_cache:
                node_detect_label = set(sum([np.array(v)[:, :3].astype(int).flatten().tolist()
                                             for v in detect_label if len(v) > 0], [])) - all_neighbour_objects
                node_glove_embeds = torch.vstack([self.vg_class_vico_embed[detect] for detect in node_detect_label]).sum(0)
                self._knowledge_cache[state.location.viewpointId] = (node_glove_embeds, node_detect_label)
            else:
                node_glove_embeds, node_detect_label = self._knowledge_cache[state.location.viewpointId]
            ob = {
                ...
                'feature': feature,
                'detect_label': node_detect_label,
                'glove_embeds': node_glove_embeds,
                'neighbour_glove_embeds': neighbour_glove_embeds,
                'neighbour_detect_label': neighbour_detect_labels,
                'rgb': rgb,
                'depth': depth,
                'candidate': candidate,
            ...
            if 'obj_vg_class_ids' in item:
                ob['obj_vg_class_ids'] = item['obj_vg_class_ids']
                ob['obj_vg_vico'] = [self.vg_class_vico_embed[cl] for cl in item['obj_vg_class_ids'] if cl != -1]
                if len(ob['obj_vg_vico']) == 0:
                    ob['obj_vg_vico'] = self.vg_class_vico_embed[-1]
