...
from map_nav_src.r2r.losses import MultiLabelCrossEntropyLoss
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0):
        ...
        self.device = torch.device('cuda:%d' % self.rank) if args.device == 'cuda' else torch.device('cpu')
        ...
        self.grad_scaler = GradScaler(enabled='cuda' in self.args.device)
        ...
        for iter in range(1, n_iters + 1):
            ...
            with autocast():
                if self.args.train_alg == 'imitation':
                    ...
                    if kwargs.get('current_iters', 0) < 0.6 * parsed_args.iters:
                        self.feedback = 'sample'
                        self.rollout(train_ml=1, train_pc=0.1, train_rl=False, **kwargs)
                    else:
                        self.feedback = 'pc_sample' if self.args.expl_sample else 'sample'
                        self.rollout(train_ml=1, train_pc=0.5, train_rl=False, **kwargs)
                else:
                    if self.args.ml_weight != 0:
                        self.feedback = 'teacher'
                        self.rollout(
    ...
    def save(self, epoch, path, score_summary=None):
        ''' Snapshot models '''
        ...
        def create_state(name, model, optimizer):
            ...
            if score_summary:
                states[name]['spl'] = score_summary['spl']
                states[name]['sr'] = score_summary['sr']
