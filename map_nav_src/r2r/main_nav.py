...
sys.path.append('/home/krishna/Projects/VLN-BEVBert/map_nav_src/')
import torch
...
from r2r.parser import parsed_args
...
def build_dataset(args, rank=0, is_test=False):
    ...
    if args.aug is not None:
        aug_instr_data = construct_instrs(
            ...
            is_test=is_test, load_objects=args.enable_path_correction
        )
    ...
    train_instr_data = train_instr_data[:args.batch_size] if args.debug else train_instr_data
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    for idx in range(start_iter, start_iter + args.iters, args.log_every):
        ...
        if aug_env is None:
            ...
            listner.train(interval, feedback=args.feedback, iter=iter)  # Train interval iters
        else:
            jdx_length = len(range(interval // 2))
            for jdx in range(interval // 2):
                # Train with GT data
                listner.env = train_env
                listner.train(1, feedback=args.feedback, current_iters=iter)
                ...
                if default_gpu:
                    print_progress(jdx, jdx_length, prefix='', suffix='Complete', bar_length=30)
        ...
        if default_gpu:
            ...
            PC_loss = sum(listner.logs['PC_loss']) / max(len(listner.logs['PC_loss']), 1)
            entropy = sum(listner.logs['entropy']) / total
            writer.add_scalar("loss/critic", critic_loss, idx)
            writer.add_scalar("policy_entropy", entropy, idx)
            writer.add_scalar("loss/RL_loss", RL_loss, idx)
            writer.add_scalar("loss/IL_loss", IL_loss, idx)
            writer.add_scalar("loss/PC_loss", PC_loss, idx)
            writer.add_scalar("total_actions", total, idx)
            writer.add_scalar("max_length", length, idx)
            write_to_record_file(
                "\ntotal_actions %d, max_length %d, entropy %.4f, IL_loss %.4f, RL_loss %.4f, PC_Loss %.4f"
                " policy_loss %.4f, critic_loss %.4f" % (
                    total, length, entropy, IL_loss, RL_loss, PC_loss, policy_loss, critic_loss),
                record_file
            )
        ...
        for env_name, env in val_envs.items():
            ...
            # Get validation distance from goal under test evaluation conditions
            listner.test(use_dropout=False, feedback='argmax', iters=None)
            argmax_preds = listner.get_results()
            argmax_preds = merge_dist_results(all_gather(argmax_preds))
            listner.test(use_dropout=False, feedback='pc_argmax', iters=None)
            pc_argmax_preds = listner.get_results()
            pc_argmax_preds = merge_dist_results(all_gather(pc_argmax_preds))

            if default_gpu:
                score_summary1, _ = env.eval_metrics(argmax_preds)
                score_summary2, _ = env.eval_metrics(pc_argmax_preds)
                loss_str += ", %s " % env_name
                for metric, val in score_summary1.items():
                    loss_str += ', %s: %.2f' % (metric, val)
                    writer.add_scalar('%s/%s' % (metric, env_name), score_summary1[metric], idx)

                loss_str += ", %s " % env_name
                for metric, val in score_summary2.items():
                    loss_str += ', %s: %.2f' % (metric, val)
                    writer.add_scalar('pc_argmax/%s/%s' % (metric, env_name), score_summary2[metric], idx)

                # select model by spl
                if env_name in best_val:
                    if args.dataset == 'r2r':
                        if ((score_summary1['sr'] + score_summary1['spl']
                                >= best_val[env_name]['sr'] + best_val[env_name]['spl']) or
                                (score_summary2['sr'] + score_summary2['spl']
                                >= best_val[env_name]['sr'] + best_val[env_name]['spl'])):
                            best_val[env_name]['spl'] = score_summary1['spl']
                            best_val[env_name]['sr'] = score_summary1['sr']
                            best_val[env_name]['state'] = 'Iter %d %s' % (iter, loss_str)
                            listner.save(idx, os.path.join(args.ckpt_dir, "best_%s" % env_name), score_summary1)
                    elif args.dataset == 'rxr':
                        if (score_summary1['sr'] + score_summary1['nDTW']
                                >= best_val[env_name]['sr'] + best_val[env_name]['nDTW']):
                            best_val[env_name]['nDTW'] = score_summary1['nDTW']
                            best_val[env_name]['sr'] = score_summary1['sr']
                            best_val[env_name]['state'] = 'Iter %d %s' % (iter, loss_str)
                            listner.save(idx, os.path.join(args.ckpt_dir, "best_%s" % env_name), score_summary1)
                    else:
...
def valid(args, train_env, val_envs, rank=-1):
    ...
    for env_name, env in val_envs.items():
        # if args.test and env_name not in 'test':
        #     continue
        prefix = f'submit' if args.detailed_output is False else 'detail'
        prefix = f'{prefix}-{args.feedback}'
        ...
        agent.test(
            use_dropout=False, feedback=args.feedback, iters=iters)
