...
def construct_instrs(anno_dir, dataset, splits, tokenizer, max_instr_len=512, is_test=True, load_objects=False):
    data = []
    object_annotations = None
    if load_objects:
        objects_path = os.path.join(anno_dir, f'OBJ_R2R_{splits[0]}.json')
        if 'prevalent' in splits[0]:
            objects_path = os.path.join(anno_dir, 'OBJ_R2R_prevalent_aug.json')
        if os.path.exists(objects_path):
            with open(objects_path, 'r') as f_:
                object_annotations = json.load(f_)
        ...
        if dataset == 'rxr':
            ...
            for j, instr in enumerate(item['instructions']):
                ...
                if object_annotations:
                    try:
                        ann = object_annotations[str(item['path_id'])]
                        new_item['obj_vg_class_ids'] = ann['obj_vgids'][j]
                        new_item['obj_vg_class_names'] = ann['obj_lemma_list'][j]
                    except KeyError:
                        ann = object_annotations[new_item['instr_id']]
                        new_item['obj_vg_class_ids'] = ann['obj_vgids'][0]
                        new_item['obj_vg_class_names'] = ann['obj_lemma_list'][0]
