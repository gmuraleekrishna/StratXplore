...
class LocalBEVEncoder(nn.Module):
    ...
    def forward(
        ...
        bev_embeds = bev_obj_embeds[:, :self.map_dim * self.map_dim]
        if obj_embeds is not None:
            obj_embeds = bev_obj_embeds[:, self.map_dim * self.map_dim:]
        else:
            obj_embeds = None
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if self.config.enable_path_correction:
            ...
            self.global_confidence_head = None
    ...
    def forward_navigation_per_step(
            ...
            detect_feats=None, detect_labels=None, obj_embeds=None, obj_masks=None
    ):
        ...
        # x, _ = self.knowledge_attention_layer(  # Attend to the feature map
        #     gmap_embeds.contiguous(),  # (batch, length, hidden) --> (batch x length, hidden)
        #     gmap_img_embeds,  # (batch, length, # of images, feature_size) --> (batch x length, # of images, feature_size)
        ...
        if self.sap_fuse_linear is None:
            ...
            CENTER = (self.map_dim * self.map_dim - 1) // 2
        ...
        # new_explore_mask = gmap_visited_masks.clone()
        # for b in range(batch_size):
        #     new_explore_mask[b, (path_state == 2).nonzero() + 1] = False
        global_logits.masked_fill_(gmap_visited_masks, -float('inf'))
        global_logits.masked_fill_(gmap_masks.logical_not(), -float('inf'))
        ...
        if self.global_confidence_head:
            confidence_scores = self.global_confidence_head(cand_embeds).squeeze(2)
            confidence_scores.masked_fill_(cand_masks.logical_not(), -float('inf'))
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            ...
            return self.forward_navigation_per_step(
                ...
                batch['bev_cand_idxs'], batch['bev_cand_vpids'], # batch['batch_cand_masks'],
                batch['detect_feats'], batch['detect_labels'],
                # obj
