...
sys.path.insert(0, '/home/krishna/Projects/Matterport3DSimulatorDocker/build_out')
...
projector, rgb_db, depth_db = None, None, None
...
def splat(state, pc_hist, pc_feat_hist, pc_mask_hist, fuse_step=1):
    ...
    for i, loc in enumerate(state.navigableLocations[1:]):
        ...
        loc_loc = (loc.x, loc.y, loc.z)
        cand_cens.append(loc_loc)
        # cand_headings.append(loc_heading)
    cand_cens = np.array(cand_cens)
    ob_pos = np.array([state.location.x, state.location.y, state.location.z])
    cand_cens = map_cand_to_bev(ob_pos, state.heading, cand_cens, MAP_DIM, MAP_RES)
    cand_pc_i = pc[:, [0, 2]].clone().repeat(cand_cens.shape[0], 1, 1)

    # Filter for high-resolution BEV
    high_res_vertex_to_map_xz = (cand_pc_i / 0.2) - cand_cens[:, None, :] + (MAP_DIM / 2)
    ...
    high_res_all_mask = pc_mask | above_mask | high_res_outside_mask

    cand_pc_feat_i = pc_feat.clone().repeat(cand_cens.shape[0], 1, 1)

    high_res_vertex_to_rgb_ft = cand_pc_feat_i[~high_res_all_mask]
    high_res_vertex_to_map_xz = high_res_vertex_to_map_xz[~high_res_all_mask]

    # Scatter features for high-res BEV
    high_res_feat_index = (MAP_DIM * high_res_vertex_to_map_xz[:, 1] + high_res_vertex_to_map_xz[:, 0]).long()

    flat_highest_y = -torch.ones(int(MAP_DIM * MAP_DIM))
    flat_highest_y, argmax_flat_spatial_map = scatter_max(
        high_res_vertex_to_rgb_ft,
        high_res_feat_index,
        dim=0,
        out=flat_highest_y,
    )
    flat_highest_y = flat_highest_y.numpy()
    argmax_flat_spatial_map = argmax_flat_spatial_map.numpy()
    bev = cand_pc_i[argmax_flat_spatial_map - 1]
    bev[flat_highest_y == -1] = 0
    bev = bev.reshape((MAP_DIM, MAP_DIM, 3)).numpy()
    return bev
...
def map_cand_to_bev(ob_pos, ob_heading, cand_poses, map_dim=None, map_res=None):
    S = np.array(ob_pos)[None, :].astype(np.float32)  # 1 x 3
    S = S[:, [0, 2, 1]] * np.array([1, 1, -1], dtype=np.float32)  # x, z, -y
    xyzhe = np.zeros([1, 5])
    xyzhe[:, 3] = -ob_heading
    T = transfrom3D(xyzhe)[0, :, :]  # 4 x 4

    cand_pos = np.array(cand_poses).astype(np.float32)
...
if __name__ == '__main__':
    ob = {'scan': 'E9uDoFAP3SH',
          'viewpoint': '2c6afdf7346a4dc3952b97edf85cf144',
          'heading': 3.665191429188092
          }
    draw_ob(ob)
