...
class ProjectorUtils():
    ...
    def pixel_to_world_mapping(self, depth_img_array, T):
        ...
        # shape: (batch_size, height, width, 3)
        pixel_to_world = torch.reshape(world_xyz, (
        (depth_img_array.shape[0], depth_img_array.shape[1], depth_img_array.shape[2], 3,)), )
...
class PointCloud(ProjectorUtils):
    ...
    def project_bev_and_cands(self, pc, no_depth_mask, pc_feat, cand_centers, high_res):
        low_res_bevs, high_res_bevs, low_res_bevs_masks, high_res_bev_masks = [], [], [], []

        map_res = self.map_res
        map_dim = self.map_dim

        # Determine the high-resolution ROI size in terms of low-resolution units
        high_res_scale = map_res / high_res
        high_res_roi_dim = map_dim / high_res_scale  # Size of the high-res ROI in low-res units

        for pc_i, no_depth_mask_i, pc_feat_i, cand_cen in zip(pc, no_depth_mask, pc_feat, cand_centers):
            # roi_center = cand_cen / high_res + (map_dim - 1) / 2
            # Low-resolution full BEV generation
            low_res_vertex_to_rgb_ft = pc_feat_i.clone()

            low_res_vertex_to_map_xz = (pc_i[:, [0, 2]] / map_res + (map_dim - 1) / 2).round()
            above_mask = (pc_i[:, 1] > 0.5)

            # Filter for low-resolution BEV
            low_res_outside_mask = (low_res_vertex_to_map_xz[:, 0] >= high_res_roi_dim) + \
                                   (low_res_vertex_to_map_xz[:, 1] >= high_res_roi_dim) + \
                                   (low_res_vertex_to_map_xz[:, 0] < 0) + \
                                   (low_res_vertex_to_map_xz[:, 1] < 0)
            low_res_all_mask = no_depth_mask_i | low_res_outside_mask | above_mask
            low_res_vertex_to_rgb_ft = low_res_vertex_to_rgb_ft[~low_res_all_mask]
            low_res_vertex_to_map_xz = low_res_vertex_to_map_xz[~low_res_all_mask]

            # Scatter features for low-res BEV
            low_res_feat_index = (
                        high_res_roi_dim * low_res_vertex_to_map_xz[:, 1] + low_res_vertex_to_map_xz[:, 0]).long()
            low_res_flat_bev = scatter_mean(low_res_vertex_to_rgb_ft,
                                            low_res_feat_index,
                                            dim=0,
                                            dim_size=map_dim * map_dim)
            low_res_bev = low_res_flat_bev.reshape(map_dim, map_dim, -1)  # Adjust the last dimension based on
            # feature size
            low_res_ob_mask = ~((low_res_bev.max(dim=-1)[0] == 0) & (low_res_bev.min(dim=-1)[0] == 0))

            low_res_bevs.append(low_res_bev)
            low_res_bevs_masks.append(low_res_ob_mask)

            # High-resolution BEV generation focused on m x m region around (a, b)
            # Translate and scale point cloud to focus on (a, b) with high resolution
            cand_pc_i = pc_i[:, [0, 2]].clone().repeat(cand_cen.shape[0], 1, 1)
            high_res_vertex_to_map_xz = (cand_pc_i / high_res) - cand_cen[:, None, :] + (map_dim / 2)

            # Filter for high-resolution BEV
            high_res_outside_mask = (high_res_vertex_to_map_xz[:, :, 0] >= map_dim) + \
                                    (high_res_vertex_to_map_xz[:, :, 1] >= map_dim) + \
                                    (high_res_vertex_to_map_xz[:, :, 0] < 0) + \
                                    (high_res_vertex_to_map_xz[:, :, 1] < 0)
            high_res_all_mask = no_depth_mask_i | above_mask | high_res_outside_mask

            cand_pc_feat_i = pc_feat.clone().repeat(cand_cen.shape[0], 1, 1)

            high_res_vertex_to_rgb_ft = cand_pc_feat_i[~high_res_all_mask]
            high_res_vertex_to_map_xz_ = high_res_vertex_to_map_xz[~high_res_all_mask]

            # Scatter features for high-res BEV
            high_res_feat_index = (map_dim * high_res_vertex_to_map_xz_[:, 1] + high_res_vertex_to_map_xz_[:, 0]).long()
            high_res_flat_bev = scatter_mean(high_res_vertex_to_rgb_ft,
                                             high_res_feat_index,
                                             dim=0,
                                             dim_size=int(map_dim * map_dim))
            high_res_bev = high_res_flat_bev.reshape(int(map_dim), int(map_dim),
                                                     -1)  # Adjust based on feature size
            high_res_ob_mask = ~((high_res_bev.max(dim=-1)[0] == 0) & (high_res_bev.min(dim=-1)[0] == 0))
            high_res_bevs.append(high_res_bev)
            high_res_bev_masks.append(high_res_ob_mask)

        # Stack all BEVs and masks
        low_res_bevs = torch.stack(low_res_bevs, dim=0)
        high_res_bevs = torch.stack(high_res_bevs, dim=0)

        return low_res_bevs, low_res_bevs_masks, high_res_bevs, high_res_bev_masks
