...
from r2r.agent_utils import calculate_confidence_scores
...
class GMapObjectNavAgent(Seq2SeqAgent):
    ...
    def _build_projector(self):
        ...
        self.bev_pos = bevpos_polar(self.map_dim).cuda()
    ...
    def _nav_bev_variable(self, obs, gmaps):
        ...
        batch_gmap_pos_fts = pad_tensors(batch_gmap_pos_fts).expand(-1, self.map_dim * self.map_dim, -1).cuda()
    ...
    def _nav_obj_variable(self, pano_embeds, view_lens, obj_lens):
        obj_embeds = [pano[view_len:view_len + obj_len].clone() for pano, view_len, obj_len in
                      zip(pano_embeds, view_lens, obj_lens)]
        ...
        for t in range(self.args.max_action_len):
            ...
            if self.feedback == 'teacher':
                ...
                if self.args.enable_path_correction:
                    recovery_confidence_scores = torch.softmax(nav_outs['confidence_scores'], 0)
                    for b in range(batch_size):
                        if ((recovery_confidence_scores[b,
                             1:len(nav_inputs['bev_cand_vpids'][b]) + 1] <= 0.5).all().item()
            ...
            for i in range(batch_size):
                if (not ended[i]) and just_ended[i]:
                    ...
                    if stop_node is not None and obs[i]['viewpoint'] != stop_node:
                        traj[i]['path'].append(gmaps[i].nav_graph.path(obs[i]['viewpoint'], stop_node))
