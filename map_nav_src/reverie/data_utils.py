...
def construct_instrs(anno_dir, dataset, splits, tokenizer, max_instr_len=512, load_objects=False):
    data = []
    object_annotations = None
    if load_objects:
        objects_path = os.path.join(anno_dir, f'OBJ_REVERIE_{splits[0]}.json')
    ...
    for i, item in enumerate(load_instr_datasets(anno_dir, dataset, splits, tokenizer)):
        ...
        for j, instr in enumerate(item['instructions']):
            ...
            if object_annotations:
                try:
                    ann = object_annotations[str(item['id'])]
